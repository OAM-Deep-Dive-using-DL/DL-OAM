...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train a DQN agent with stable rewards for OAM handover decisions")
    ...
    parser.add_argument("--stable-config", type=str, default="stable_reward_config_new",
...
def train(args: argparse.Namespace) -> None:
    """
    Train the DQN agent with stable rewards.
    ...
    if args.config.endswith('.yaml'):
        ...
        stable_config = load_config(args.stable_config)
        
        # Merge configurations
        config = merge_configs(sim_config, rl_config)
        config = merge_configs(config, stable_config)
    else:
        # Hierarchical config
        # Just load the stable reward config which inherits from rl_config_new
        config = load_hierarchical_config(args.stable_config)
    
    # Create output directory with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join(args.output_dir, f"train_stable_{timestamp}")
    ...
    # Create environment with stable rewards
    env = StableOAM_Env(config)
    ...
    episode_throughputs = []
    episode_handovers = []
    episode_avg_rewards = []  # Track moving average of rewards
    
    print(f"Starting training with stable rewards for {num_episodes} episodes with max {max_steps} steps per episode...")
    ...
    for episode in range(1, num_episodes + 1):
        ...
        episode_throughputs.append(env.episode_throughput)
        episode_handovers.append(env.episode_handovers)
        
        # Calculate moving average of rewards (for more stable tracking)
        ...
        metrics_logger.log_scalar("avg_reward", avg_reward, episode)
        metrics_logger.log_scalar("throughput", env.episode_throughput, episode)
    ...
    metrics = {
        ...
        "avg_rewards": episode_avg_rewards,
