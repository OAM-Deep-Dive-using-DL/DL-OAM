...
class ChannelSimulator:
    ...
    def _validate_parameters(self):
        ...
        if not (50 <= self.pressure <= 120):
            errors.append(f"Pressure {self.pressure} kPa is outside reasonable range (50-120 kPa)")
    ...
    def _calculate_atmospheric_absorption(self, distance: float) -> float:
        ...
        # ITU-R P.676 atmospheric absorption coefficients
        # Oxygen absorption (dB/km) - simplified model for 28 GHz
        if freq_GHz < 60:
            # Below 60 GHz: oxygen absorption increases with frequency
            gamma_oxygen = 0.0006 * (freq_GHz**2) * (self.pressure/101.3)
        else:
            # Above 60 GHz: stronger molecular resonances
            gamma_oxygen = 0.008 * freq_GHz * (self.pressure/101.3)
        
        # Water vapor absorption (dB/km) - depends on humidity and temperature
        # Simplified model based on ITU-R P.676
        if freq_GHz < 60:
            # Below 60 GHz: water vapor absorption
            gamma_water = 0.0001 * freq_GHz * self.humidity * np.exp(self.temperature/15)
        else:
            # Above 60 GHz: stronger water vapor absorption
