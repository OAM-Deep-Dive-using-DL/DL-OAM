# #!/usr/bin/env python3
# """
# IEEE Format Figure 1: System Model & Problem Definition (FIXED v2)
# ==================================================================
...
# Fixes:
# - Corrected ValueError in Poly3DCollection by providing full 3D coordinates for vertices.
# - Corrected variable name typo from IEEE_GEO_COLORS to IEEE_COLORS.
# - Replaced inefficient loops with vectorized NumPy operations.
# - Implemented a clearer visualization strategy for OAM beams using cross-sections.
# - Replaced cluttered turbulence surfaces with a more intuitive 3D scatter plot.

# Author: OAM 6G Research Team (revised by AI assistant)
# Date: 2024
# """
...
# def get_lg_beam_intensity(l: int, r: np.ndarray, w_z: float) -> np.ndarray:
#     """
#     Calculates the normalized intensity of a Laguerre-Gaussian beam.
#     This is the core physics of the OAM beam's cross-section.
#     """
#     # Correctly model the radial dependency for l=0 and l>0
#     if l == 0:
#         # Fundamental Gaussian beam profile
#         amplitude_profile = np.exp(-r**2 / w_z**2)
#     else:
#         # Donut shape for higher-order modes
#         amplitude_profile = (np.sqrt(2) * r / w_z)**(2 * abs(l)) * np.exp(-2 * r**2 / w_z**2)
    
#     # Intensity is the square of the amplitude
#     intensity = amplitude_profile
...
#     x, y, z = pos
#     # User icon (Unicode character)
#     ax.text(x, y, z, "📱", fontsize=20, ha='center', va='center', zorder=10)
#     # Movement trajectory
#     traj_x = np.linspace(x - 100, x, 50)
#     traj_y = np.sin(np.linspace(0, 2*np.pi, 50)) * 5 # Slight curve for visual interest
...
#     This version provides full 3D coordinates to Poly3DCollection to fix the ValueError.
#     """
...
#     # Path of the beam center
#     ax.plot(x_points, y_points, z_points, '-', color=color, linewidth=0.5, alpha=0.6)

#     # Draw cross-sections (rings) to visualize the beam shape
#     theta = np.linspace(0, 2 * np.pi, 50)
#     for i in range(num_slices):
#         dist_from_bs = x_points[i]
#         w_z = get_beam_width(dist_from_bs)
        
#         radius = w_z * np.sqrt(abs(l) / 2) if l != 0 else 0.1 * w_z
#         path_loss_factor = 1 / (1 + (dist_from_bs/150)**2)
        
#         # <<< FIX: Create a full set of 3D coordinates for the ring polygon >>>
#         # The x-coordinate is constant for all points in a single ring slice.
#         ring_x = np.full_like(theta, x_points[i])
#         # The y- and z-coordinates define the circle in the y-z plane.
#         ring_y = radius * np.cos(theta) + y_points[i]
#         ring_z = radius * np.sin(theta) + z_points[i]
        
#         # Combine into a list of 3D points [(x1,y1,z1), (x2,y2,z2), ...]
#         # This is the correct format for Poly3DCollection
#         verts = [list(zip(ring_x, ring_y, ring_z))]
        
#         # Use Poly3DCollection with the now-correct 3D vertices
#         poly = Poly3DCollection(verts, facecolors=color, alpha=0.5 * path_loss_factor, linewidths=0)
        
#         # Add the 3D collection directly to the axes
...
#         if mode not in [1, 3]: # Only draw paths for non-visualized modes
...
#     This FIX is clearer and less cluttered than overlapping surfaces.
#     """
#     # Generate random points in a volume along the beam path
#     num_eddies = 200
#     x = np.random.uniform(20, path_length - 20, num_eddies)
#     y = np.random.uniform(-15, 15, num_eddies)
#     z = np.random.uniform(0, 15, num_eddies)
    
#     # Eddy size and color intensity based on turbulence strength ($C_n^2$)
#     # We simulate stronger turbulence further from the BS
#     sizes = (1 + (x / path_length)**2) * 50 
#     colors = x / path_length # Use distance to map color, warmer = stronger turbulence
...
#     arrow_props = dict(arrowstyle="->", color=IEEE_COLORS['red'], lw=1.5, connectionstyle="arc3,rad=0.2")
#     ax.annotate("", xy=(100, 5), xytext=(125, 20),
#                 arrowprops=arrow_props,
#                 xycoords=art3d.proj_transform(100, 5, 5, ax.get_proj()),
...
#     # Save the figure
#     output_filename = 'Figure1_SystemModel_Fixed.png'
...
#!/usr/bin/env python3
"""
IEEE Format Figure 1: System Model & Problem Definition (FIXED v3)
==================================================================
...
Fixes:
- Replaced deprecated `art3d.proj_transform` with `ax.quiver` for stable 3D annotations.
- Corrected ValueError in Poly3DCollection by providing full 3D coordinates for vertices.
- Corrected variable name typo from IEEE_GEO_COLORS to IEEE_COLORS.
- Replaced inefficient loops with vectorized NumPy operations.
- Implemented a clearer visualization strategy for OAM beams using cross-sections.
...
def add_annotations(ax: Axes3D):
    ...
    # <<< FIX: Replaced deprecated annotate call with a direct 3D arrow using quiver >>>
